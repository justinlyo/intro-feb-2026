...
public class CalculatorInteractionTests
{
    ...
    public void LoggerThrows(string example, string expected)
    {
        ...
        var mockedOutput = Substitute.For<IOutput>();
        var calculator = new Calculator(
            stubbedLogger, 
            mockedNotifier,
            mockedOutput
        );
        stubbedLogger.When(logger => logger.Write(Arg.Any<string>()))
    ...
    public void HelpDeskNotnotifiedWithNoException(string example, string expected)
    {
        ...
        var mockedNotifier = Substitute.For<INotifyTheHelpDesk>();
        var mockedOutput = Substitute.For<IOutput>();
        var calculator = new Calculator(stubbedLogger, mockedNotifier, mockedOutput);
    ...
    [Theory]
    [InlineData("1,2", "3")]
    public void WriteResultToConsole(string example, string expected)
    {
        //// Given
        var stubbedLogger = Substitute.For<ILogger>();
        var mockedNotifier = Substitute.For<INotifyTheHelpDesk>();
        ...
        //When
        var calc = new Calculator(stubbedLogger, mockedNotifier, mockedOutput);
        calc.Add(example);
        // Then
        mockedOutput.Received(1).WriteLine(expected);
    }
}
